<!-- ===== NAVBAR ===== -->
  <header class="fixed top-0 left-0 right-0 z-50 nav-blur border-b border-white/5">
    ...
    <a href="{{ route('home') }}"
       class="{{ request()->routeIs('home') ? 'text-brand font-medium' : 'text-slate-300 hover:text-white' }} transition-colors">Home</a>

    <a href="{{ route('about-us') }}"
       class="{{ request()->routeIs('about-us') ? 'text-brand font-medium' : 'text-slate-300 hover:text-white' }} transition-colors">About Us</a>

    <a href="{{ route('our-services') }}"
       class="{{ request()->routeIs('our-services') || request()->routeIs('services.single') ? 'text-brand font-medium' : 'text-slate-300 hover:text-white' }} transition-colors">Our Services</a>

    <a href="{{ route('our-teams') }}"
       class="{{ request()->routeIs('our-teams') ? 'text-brand font-medium' : 'text-slate-300 hover:text-white' }} transition-colors">Team</a>

    <a href="{{ route('clientele') }}"
       class="{{ request()->routeIs('clientele') ? 'text-brand font-medium' : 'text-slate-300 hover:text-white' }} transition-colors">Clientele</a>

    <a href="#testimonials"
       class="text-slate-300 hover:text-white transition-colors">Testimonials</a>
</nav>
