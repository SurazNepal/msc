<div class="p-6 w-full bg-white dark:bg-zinc-900 border border-zinc-200 dark:border-zinc-800 rounded-xl shadow-sm">
    <form wire:submit.prevent="save" enctype="multipart/form-data" class="space-y-6">
        ...
        <div class="pb-3 border-b border-zinc-200 dark:border-zinc-800">
            <h1 class="text-xl font-bold text-zinc-900 dark:text-white">Add New Custom Page</h1>
            <p class="text-xs text-zinc-500">Configure parameters to offer a custom runtime core template context.</p>
        </div>
        ...
        <div class="grid grid-cols-1 gap-4" x-data= "{ isUploading: false, progress: 0 }">
            ...
            <div class="space-y-2"
                ...
                <flux:input type="file" size="sm" badge="Max 2MB" name="featuredImage" id="featuredImage" wire:model="featuredImage" label="Featured Banner Image Asset"/>
        ...
        <div class="flex justify-end gap-2 pt-4 border-t border-zinc-200 dark:border-zinc-800">
            ...
            <flux:button type="submit" variant="primary">Save Custom Page</flux:button>
        </div>
    </form>
</div>
