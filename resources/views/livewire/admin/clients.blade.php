                ...
                <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                    ...
                    <div x-data="{ isUploading: false, progress: 0 }"
                        ...
                        <div class="flex flex-col">
                            <label class="text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">Client Logo / Badge</label>
                            <input type="file" id="client-file-{{ $thumbnail ? 'active' : 'empty' }}" wire:model="thumbnail" class="block w-full text-sm text-gray-900 border border-gray-300 rounded-lg cursor-pointer bg-gray-50 focus:outline-none focus:ring-2 focus:ring-primary-500 dark:text-gray-400 dark:bg-gray-700 dark:border-gray-600" />
                        </div>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 gap-4 mt-2">
                            <div x-show="isUploading" class="col-span-full w-full mt-2" x-cloak>
                                ...
                                <p class="text-xs text-gray-500 mt-1">Uploading asset... <span x-text="progress"></span>%</p>
                            </div>

                            @if ($thumbnail)
                                <div wire:key="temp-client-thumb" class="relative aspect-[16/10] border rounded-lg overflow-hidden shadow group p-2 bg-gray-50 dark:bg-zinc-900 flex items-center justify-center">
                                    <img src="{{ $thumbnail->temporaryUrl() }}" class="max-w-full max-h-full object-contain" />
                                    <button type="button" wire:click="removeUpload" class="absolute top-2 right-2 z-50 bg-red-500 text-white rounded-full p-1.5 hover:bg-red-600 focus:outline-none transition-all duration-150 shadow-md" title="Cancel file choice">
                                        <svg xmlns="http://www.w3.org/2000/svg" class="h-4 w-4" fill="none" viewBox="0 0 24 24" stroke="currentColor"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M6 18L18 6M6 6l12 12" /></svg>
                                    </button>
                                </div>
                            @elseif ($isEditMode && $existingThumbnail)
                                <div wire:key="stored-client-thumb" class="relative aspect-[16/10] border rounded-lg overflow-hidden shadow group p-2 bg-gray-50 dark:bg-zinc-900 flex items-center justify-center">
                                    <img src="{{ $existingThumbnail }}" class="max-w-full max-h-full object-contain" />
                                    <button type="button" wire:click="deleteExistingThumbnail" class="absolute top-2 right-2 z-50 bg-red-600 text-white rounded-full p-1.5 hover:bg-red-700 shadow-md focus:outline-none transition-all duration-150 flex items-center justify-center" title="Delete logo asset from server">
                    ...
                    <tr class="text-center bg-white border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                        ...
                        <td class="px-6 py-4 text-center font-medium text-blue-600 dark:text-blue-400 max-w-xs truncate">
                            @if($client->website_url)
                                <a href="{{ $client->website_url }}" target="_blank" class="hover:underline flex items-center gap-1">
                                    {{ $client->website_url }}
                                </a>
                            @else
